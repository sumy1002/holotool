...
DEFAULT_CONFIG = {
    ...
    # 選牌畫面「五張沒認齊」時，最多停在那裡幾秒才自救。
    #
    # 原本沒有這個機制：只要有一格認不出來，bot 就永遠停在選牌畫面
    # （一張鬼牌就足以讓整晚的任務停擺）。超過這個秒數之後，程式會**只用
    # 認得出來的牌**做最保守的決定（有對子就留對子，否則五張全換）再按「替換」，
    # 讓流程繼續跑，並在 log 大聲說明。
    #
    # 判讀只要有任何一格改變，計時就重新開始 —— 發牌動畫、翻牌過渡影格那種
    # 「暫時認不出來」不會誤觸發。
    # 設成 0 = 完全關掉自救，回到「認不齊就停著等你處理」。
    "partial_draw_rescue_sec": 20.0,
